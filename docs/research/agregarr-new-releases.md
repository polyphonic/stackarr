# Agregarr newest-release rows research
...
Create two Agregarr `filtered_hub` configs: **New Movies** for the movie library and **New Episodes** for the TV library. Both use subtype `recently_released`. For Movies, that means movie release date; for TV, it means show-level ordering by the most recent episode's air date (`episode.originallyAvailableAt:desc`), not the show's debut date. Do not use the separate `recently_released_episodes` subtype for New Episodes: in Agregarr 2.4.2 that subtype orders TV shows by episode **added-to-Plex** time (`episode.addedAt:desc`), not episode air date. Promote both rows to Home, Shared Users' Home, and each library's Recommended screen. Keep randomization off and put each row first in its library's Home/Recommended and promoted-library ordering.

This is preferable to directly enabling Plex's built-in hubs for this Stackarr configuration:

- Agregarr can discover and manage Plex's built-in `movie.recentlyreleased` (`Recently Released Movies`) and `tv.recentlyaired` (`Recently Released Episodes`) hubs. Upstream explicitly notes that Plex has no built-in `tv.recentlyreleased` show hub. The available native hubs therefore have different identifiers and content shapes. Agregarr also deliberately preserves a default hub's Plex-provided name, identifier, media type, and per-library sort positions when settings are updated, so it cannot rename those native hubs to `New Movies` and `New Episodes` through the settings endpoint. [Agregarr native-hub mapping](https://github.com/agregarr/agregarr/blob/v2.4.2/src/components/Collections/Views/CollectionSettings.tsx#L111-L121), [built-in hub identifiers](https://github.com/agregarr/agregarr/blob/v2.4.2/server/lib/collections/utils/HubIdentifierUtils.ts#L250-L273), [default-hub update behavior](https://github.com/agregarr/agregarr/blob/v2.4.2/server/lib/collections/services/DefaultHubConfigService.ts#L112-L175)
...
Agregarr's authenticated creation endpoint supports multiple `libraryIds`, but a multi-library request shares one display name. Because these two rows intentionally have different names, create each library config separately with one `libraryIds` entry. Agregarr infers movie versus TV from the selected Plex library. Non-franchise collections default to the promoted library section, and new configs without an existing sort position are auto-reordered to the front in both Home and library contexts. [Creation route](https://github.com/agregarr/agregarr/blob/v2.4.2/server/routes/collections.ts#L1370-L1525), [creation reordering](https://github.com/agregarr/agregarr/blob/v2.4.2/server/routes/collections.ts#L1545-L1584), [auto-reorder semantics](https://github.com/agregarr/agregarr/blob/v2.4.2/server/routes/reorder.ts#L423-L479)
...
1. `GET /api/v1/collections` and match `New Movies` to the movie library and `New Episodes` to the TV library, both with `type == "filtered_hub"` and `subtype == "recently_released"`. Also recognize the former `New Releases` and native-style names during migration.
...
3. Update each existing config through `PUT /api/v1/collections/{id}/settings`, preserving its full current document while enforcing the media-specific name and fields above. The endpoint intentionally preserves library-specific rating keys and sort positions. [Collection update behavior](https://github.com/agregarr/agregarr/blob/v2.4.2/server/routes/collections.ts#L653-L689)
...
- Agregarr's `/api/v1` routes are its web application's native API, not a separately versioned external contract. Because Stackarr tracks `agregarr:latest`, keep source-contract tests for request fields, media-specific names, sort direction, visibility, and reorder behavior.
...
- Validate both Plex library rows separately: exact titles `New Movies` and `New Episodes`; the movie smart query sorts on `originallyAvailableAt:desc`; the TV smart query sorts on `episode.originallyAvailableAt:desc` rather than the show's own `originallyAvailableAt`; no `Trailer (Placeholder)` item is present; at most `maxItems` items appear; and each row is first in its library's Recommended section and Home rows.
