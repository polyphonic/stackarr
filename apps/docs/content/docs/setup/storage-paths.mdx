...
| `MUSIC_ROOT` | Managed music library root mounted into Lidarr as `/music`. Lidarr receives the access required to validate and index the root while completed-download handling and automatic track renaming stay off for manual library management. |
