...
- YouTube library: `${YOUTARR_OUTPUT_ROOT}` → `/usr/src/app/data` (new installs default to `${MEDIA_ROOT}/Videos/YouTube`)
...
Youtarr writes its channel name into the MP4 `album` tag, which Plex uses for the per-channel collection. Music videos can follow the same convention by setting `album` to the artist name; Agregarr is not needed for either grouping model. See [Storage Paths](/docs/setup/storage-paths#folder-organized-videos) for the combined folder and collection layout.
