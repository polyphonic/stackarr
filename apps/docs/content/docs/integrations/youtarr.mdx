...
- App image: `dialmaster/youtarr:latest`
...
- Queue one validated YouTube URL or video ID for download.
...
Stackarr backups include the persistent Youtarr files and a consistent MariaDB dump. If Youtarr is enabled but its database cannot be dumped, the backup fails rather than creating an incomplete archive. Restore presents Youtarr database recovery as a separate option.
