...
```bash
bin/stackarr cloudflare install \
  --api-token <cloudflare-api-token> \
  --route books.example.com=bookorbit:access \
  --route photos.example.com=immich:public \
  --route games.example.com=romm:access \
  --access-email family@example.com
```
