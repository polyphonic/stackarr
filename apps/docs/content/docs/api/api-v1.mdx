...
| `POST` | `/api/v1/backup/recovery-key` | Download the current backup recovery key from an authenticated dashboard session after current-password reauthentication. |
...
The recovery-key download is intentionally stricter than ordinary API-key operations: it requires a signed-in browser session and the current admin password, disables response caching, and is not available to API-key-only clients or agents.
