...
Mutating endpoints require `X-Api-Key`. CLI setup and the first dashboard config save create an API key; command endpoints fail closed when no key is configured.
