...
browser_service_url() {
    ...
    if [[ "${STACKARR_SERVICE_URL_MODE:-localhost}" == "portless" ]]; then
        printf '%s://%s.%s\n' "${STACKARR_SERVICE_URL_SCHEME:-https}" "$service" "${STACKARR_SERVICE_URL_HOST_SUFFIX:-stackarr}"
        return 0
    fi
...
load_env() {
    : "${STACKARR_DATABASE_FILE:=$REPO_ROOT/stackarr/config/stackarr.db}"
    load_sqlite_runtime_config
    load_browser_link_runtime_settings
    ...
    : "${BOOKORBIT_POSTGRES_PASSWORD:=${PASSWORD:-$(random_secret 24)}}"
    ...
    : "${DATABASE_SUPERUSER_PASSWORD:=${PASSWORD:-$(random_secret 24)}}"
...
include = re.compile(
    ...
    r"STACKARR_.*|ENABLE_.*|PLEX_.*|JELLYFIN_.*|BOOKORBIT_.*|DATABASE_.*|SEERR_.*|PULSARR_.*|BAZARR_.*|"
...
compose_profile_args() {
    ...
    for profile in movies tv radarr4k sonarr4k bazarr lidarr bookorbit tinymediamanager recyclarr flaresolverr tidarr seerr pulsarr; do
...
remove_inactive_torrent_client_container() {
    ...
    docker compose -f "$ROOT_DIR/docker-compose.yml" --profile "$inactive" rm -f -s "$inactive" >/dev/null 2>&1 || true
}

remove_disabled_optional_containers() {
    local service

    for service in movies tv radarr4k sonarr4k bazarr lidarr bookorbit tinymediamanager recyclarr flaresolverr tidarr seerr pulsarr; do
        if ! optional_service_enabled "$service"; then
            docker compose -f "$ROOT_DIR/docker-compose.yml" --profile "$service" rm -f -s "$service" >/dev/null 2>&1 || true
        fi
    done
}
...
refresh_stackarr_web_storage_mounts() {
    ...
    if ! docker compose -f "$ROOT_DIR/docker-compose.yml" "${compose_args[@]}" ps --services --status running 2>/dev/null | grep -qx 'stackarr'; then
        return 0
    fi
    ...
    for path in "${MEDIA_ROOT:-}" "${MUSIC_ROOT:-}" "${DOWNLOADS_ROOT:-}" "${BACKUP_ROOT:-}"; do
        ...
        if ! docker compose -f "$ROOT_DIR/docker-compose.yml" "${compose_args[@]}" exec -T stackarr test -e "$path" >/dev/null 2>&1; then
            missing+=("$path")
        fi
    done
    ...
    write_compose_env_file
    docker compose -f "$ROOT_DIR/docker-compose.yml" "${compose_args[@]}" up -d --force-recreate --no-deps stackarr
}
