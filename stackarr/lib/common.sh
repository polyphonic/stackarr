...
include = re.compile(
    ...
    r"STACKARR_.*|ENABLE_.*|PLEX_.*|JELLYFIN_.*|BOOKORBIT_.*|IMMICH_.*|ROMM_.*|DATABASE_.*|SEERR_.*|PULSARR_.*|MAINTAINERR_.*|AGREGARR_.*|TRACEARR_.*|BAZARR_.*|"
...
compose_profile_args() {
    ...
    for profile in movies tv radarr4k sonarr4k bazarr lidarr bookorbit immich romm tinymediamanager recyclarr flaresolverr tidarr seerr pulsarr maintainerr agregarr tracearr; do
...
remove_disabled_optional_containers() {
    ...
    for service in movies tv radarr4k sonarr4k bazarr lidarr bookorbit immich romm tinymediamanager recyclarr flaresolverr tidarr seerr pulsarr maintainerr agregarr tracearr; do
